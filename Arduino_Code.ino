...
17 WITH IMPROVED SERIAL PORT SELECTOR TOOL
...
const int buzzer = 9; //buzzer to arduino pin 9
 ...
 pinMode(buzzer, OUTPUT);
  ...
  while(! mpu.begin( Serial.begin(9600)))
  {
    Serial.println("Could not find a valid Pulse sensor, check wiring!");
...
int g= absolute/ 60000;
int h= boolea;
  // calculate temperature for LM34
  //temperatureF = voltageOut / 10;
  //temperatureC = (temperatureF - 32.0)*(5.0/9.0);
  
  //Heartbeat taken from pulse sensor
  //Temperature sensor has linearity error
  
 int heart= g*60;
 int temperature=  h+6;
 int heartbeat= heart*1000;
 
  
 if (heartbeat>60 && temperature>95 )
            {

                  tone(buzzer, 1); // Send 1KHz sound signal..
                  noTone(buzzer); 
                int statusSensor = digitalRead (soundSensor);     //give frequency of sound
                  if (statusSensor == 1)
                  {
                            Serial.println(1);
